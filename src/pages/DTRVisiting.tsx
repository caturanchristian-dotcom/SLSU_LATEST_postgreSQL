...
const parseLocalDateNoShift = (dateStr: any): Date | null => {
...
const DTRVisiting = () => {
  ...
  const fetchSchedulesForEmployee = (empId: string) => {
    if (empId) {
      fetch(`/api/schedules/employee/${empId}`)
        .then(res => res.json())
        .then(data => setEmployeeSchedules(Array.isArray(data) ? data : []))
  ...
  const handleOpenEditSchedule = (sch: any) => {
    setEditSchedule({
      ...
      specificDate: sch.specificDate ? safeDateOnly(sch.specificDate) : '',
      effectiveFrom: sch.effectiveFrom ? safeDateOnly(sch.effectiveFrom) : '',
      effectiveTo: sch.effectiveTo ? safeDateOnly(sch.effectiveTo) : ''
    });
  ...
  const fetchOvertime = useCallback(async (month = selectedMonth, year = selectedYear, empId = selectedEmployeeId) => {
    try {
      ...
      console.error('Failed to fetch approved overtime in DTRVisiting:', err);
      setApprovedOvertimes([]);
    }
  }, [isAdmin, user?.id, selectedMonth, selectedYear, selectedEmployeeId]);
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      const filtered = emps.filter((emp: Employee) => emp.category === 'Visiting Instructor');
    ...
    const initData = async () => {
      ...
      try {
        ...
        if (bootstrap.employees && bootstrap.employees.length > 0) {
          const filtered = bootstrap.employees.filter((emp: Employee) => emp.category === 'Visiting Instructor');
          setEmployees(filtered);
        ...
        console.error("Bootstrap loading error in DTRVisiting:", e);
      } finally {
  ...
  useRealtime('employees_changed', () => {
    if (isAdmin) fetchEmployees();
  });
  useRealtime('schedules_changed', () => {
    if (selectedEmployeeId) fetchSchedulesForEmployee(selectedEmployeeId);
  });
  ...
  const getSchedulesForEditDate = () => {
    ...
    try {
      ...
      return employeeSchedules.filter((sch: any) => {
        const dateVal = safeDateOnly(editDate);
        if (sch.effectiveFrom && dateVal < safeDateOnly(sch.effectiveFrom)) return false;
        if (sch.effectiveTo && dateVal > safeDateOnly(sch.effectiveTo)) return false;
  ...
  const getDayPunchesInner = (dayLogs: DTRLog[], dateStrOverride?: string) => {
    ...
    const formatPunchTime = (val: string | null | undefined, baseDate?: any) => {
      ...
      if (/^\d{1,2}:\d{2}(:\d{2})?$/.test(str)) {
        ...
        const d = baseDate ? parseLocalDateNoShift(`${safeDateOnly(baseDate)}T${picker}:00`) : null;
    ...
    const activeDateStr = dateStrOverride ? safeDateOnly(dateStrOverride) : (dayLogs[0] ? safeDateOnly(dayLogs[0].date) : null);
    ...
    if (sorted.length >= 2) {
      ...
      if (primaryAM.amIn || primaryAM.amOut || primaryAM.pmIn || primaryAM.pmOut) {
        if (primaryAM.amIn) {
          ...
          amInVal = res.display;
          pickerAmInVal = res.picker;
          amInDate = res.date;
        }
        if (primaryAM.amOut) {
          const res = formatPunchTime(primaryAM.amOut, activeDateStr || undefined);
          amOutVal = res.display;
          pickerAmOutVal = res.picker;
          amOutDate = res.date;
        }
      } else {
      ...
      if (primaryPM.pmIn || primaryPM.pmOut) {
        if (primaryPM.pmIn) {
          ...
          pmInVal = res.display;
          pickerPmInVal = res.picker;
          pmInDate = res.date;
        }
        if (primaryPM.pmOut) {
          const res = formatPunchTime(primaryPM.pmOut, activeDateStr || undefined);
          pmOutVal = res.display;
          pickerPmOutVal = res.picker;
          pmOutDate = res.date;
        }
      } else {
    ...
    matchingSchedules.forEach((sch: any) => {
      if (sch.startTime && sch.endTime) {
        let sh = Number(safeSplit(sch.startTime, ':')[0]);
        let sm = Number(safeSplit(sch.startTime, ':')[1] || 0);
        if (sh > 0 && sh <= 6) sh += 12;

        let eh = Number(safeSplit(sch.endTime, ':')[0]);
        let em = Number(safeSplit(sch.endTime, ':')[1] || 0);
        if (eh > 0 && eh <= 6) eh += 12;
  ...
  const getDayPunches = (dayLogs: DTRLog[], dateStrOverride?: any) => {
  ...
  const getScheduledHoursForDate = (dateStr: any) => {
    const dateOnly = safeDateOnly(dateStr);
    const parts = safeSplit(dateOnly, '-');
  ...
  const { fullMonthRows, fullMonthTotals } = useMemo(() => {
    const daysInMonth = new Date(selectedYear, selectedMonth, 0).getDate();
    const rows: any[] = [];
    const daysOfWeekStr = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    ...
    return { fullMonthRows: rows, fullMonthTotals: mTotals };
  }, [sheetEntries, selectedYear, selectedMonth, holidays, employeeSchedules]);
              ...
              <tbody className="font-mono text-[10px] text-neutral-900 leading-tight">
                {fullMonthRows.map(row => {
                  ...
                  return (
                    ...
                    >
                      <td className="border-r border-black font-extrabold py-0.5 w-7 bg-neutral-50/70 select-none text-[9.5px] print-cell-border text-center relative group">
                        {row.day}
                        {row.holiday && (
                      ...
                      {row.holiday && !hasPunches ? (
                        ...
                        <>
                          ...
                          <td className="border-r border-black py-0.5 text-center print-cell-border text-neutral-900 font-semibold font-mono">
                            {row.undertimeMin}
                          </td>
                          <td 
                            onClick={(e) => {
        ...
        <div className="mt-3 space-y-2.5 border-t border-black pt-2 font-sans text-neutral-850">
          ...
          <div className="pt-2.5 text-center">
            <span className="text-[11px] font-black uppercase tracking-wider block">
              {nameData.firstName ? nameData.firstName.toUpperCase() : ''}&nbsp;{nameData.middleName ? nameData.middleName.charAt(0).toUpperCase() + '.' : ''}&nbsp;{nameData.lastName ? nameData.lastName.toUpperCase() : ''}
            </span>
            ...
            <>
              <SchoolApiSyncModal
                ...
                triggerButton={
                  <Button 
                    variant="outline" 
                    className="gap-2 border-emerald-300 text-emerald-800 bg-emerald-50/70 hover:bg-emerald-100 rounded-xl font-sans text-xs h-10 px-4 shadow-sm"
                  >
                    ...
                    <div key={sch.id} className="bg-white border border-neutral-100 p-2.5 rounded-xl text-xs flex items-center justify-between gap-2 shadow-sm">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Button
                          ...
                          onClick={() => {
                            const [sh] = safeSplit(sch.startTime, ':').map(Number);
